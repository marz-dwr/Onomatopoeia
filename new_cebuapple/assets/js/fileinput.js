...
(function ($) {
    ...
    var isIE = function (ver) { // check for IE versions < 11
        ...
        tActionUpload = '<button type="button" class="kv-file-upload {uploadClass}" title="{uploadTitle}">' +
            '   {uploadIcon}\n</button>\n',
        //zoom has been hidden to avoid modal disruption
        tZoom = '<button type="button" class="btn btn-default btn-xs btn-block hidden" title="{zoomTitle}: {caption}" onclick="{dialog}">\n' +
            '   {zoomInd}\n' +
            '</button>\n',
